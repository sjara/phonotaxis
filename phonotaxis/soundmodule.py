...
def apply_rise_fall(waveform, samplingRate, riseTime, fallTime):
    ...
    if (len(newWaveform)>nSamplesRise) and (len(waveform)>nSamplesFall):
        newWaveform[:nSamplesRise] *= riseVec[:,np.newaxis]
        newWaveform[-nSamplesFall:] *= fallVec[:,np.newaxis]
    return newWaveform
...
class AlsaPlayer():
    ...
    def _open_pcm(self):
        ...
        pcm = self._alsaaudio.PCM(
            ...
            format=self._alsaaudio.PCM_FORMAT_S32_LE,
            periodsize=self.period_size,
        )
        actual_rate = pcm.info()['rate']
